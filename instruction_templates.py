...
def build_instruction(task, text=None, options=None, region=None, context=None, question=None, explanation=None, response=None, premise=None, hypothesis=None, answer=None, meta_data=None, target=None, use_natural=False, instruction_id=-1):
    ...
    # ----------------------- testing tasks ----------------------------- #
        ...
        instructs = [
            f"""In this task, you are given a short sentence and a image. You need to decide if the content of the image can support the text. Text: {text} {options_str}""",
            f"""Can you conclude \"{text}\" from the content of image? Select your answer from the options.{options_str}""",
            f"""Can you conclude \"{text}\" from the content of image?{options_str}""",
            f"""Can the content of the image support \"{text}\"?{options_str}""",
            f"""Does the image support the given text?\nText: {text}{options_str}"""
        ]
    elif task == 'natural_language_visual_reasoning':
        instructs = [
            f"""Does the content of the image support the given text? Text: {text}{options_str}""",
            f"""Based on the image, is \"{text}\" true?{options_str}""",
            f"""Decide if the content of the image supports the sentence. Sentence:{text}{options_str}""",
            f"""\"{text}\"\n\nIs the above text true based on the picture?{options_str}""",
            f"""Look at the picture and is \"{text}\" true?{options_str}""",
        ]
        ...
        instructs = [
            ...
            f"""In this task, you need to decide if the positional relationship in the text is true, based on the image. The text is: {text}{options_str}""",
            f"""Does the content of the image support the sentence below?\n\nSentence:{text}{options_str}""",
            f"""Can the image support \"{text}\"?{options_str}""",
            f"""Is \"{text}\" true, by referring to the image?{options_str}""",
        ]
    ...
    # ----------------------------------------- VQA
        ...
        instructs = [
            ...
            f"""There is some text on the image. Answer {question} based on the text in image.""",
            f"""Look at the text on image and answer: {question}""",
            f"""Look at the image and {question}""",
            f"""{question}""",
        ]
    elif task == 'grounded_VQA':
        instructs = [
            f"""In this task, you are given a question and you need to identify a region from options in order to answer it. The question is: {question}{options_str}""",
            f"""Select a region from the options to answer \"{question}\"{options_str}""",
            f"""Which region can be used to answer \"{question}\"{options_str}""",
            f"""Which region is the answer to \"{question}\"{options_str}""",
            f"""{question}{options_token} {split_token.join(options)}""",
        ]
        ...
        instructs = [
            f"""What is the text in the given region {region[0]} in the {image_token}?""",
            f"""Look at image and answer what is the text in {region[0]}?""",
            f"""In this task, you are require to extract the text in a region of the image. The region is {region[0]}. What is the text?""",
            f"""What is the text in the given region of the {image_token}. {region_token} {split_token.join(region)}""",
            f"""What is the text written on {split_token.join(region)} of the image.""",
        ]
        ...
        instructs = [
            ...
            f"""Given the image and question: \"{question}\" {region_info} Select an explanation from options to exlpain why \"{answer}\" is the answer.{options_str}""",
        ]
    # ------------------------------- misc
        ...
        if len(dial_history) > 0:
            ...
            instructs = [
                ...
                f"""Based on the image, answer: {question}?""",
                f"""Based on the image, answer {question}?""",
                f"""Given the image, {question}?""",
                f"""{question}?"""
            ]
        ...
        instructs = [
                     ...
                     f"""Considering both the content of the image and the text on the image, decide if it contains hateful intension.{options_str}"""
                     f"""Look at the image and the text on the image. Decide if there is hateful    intention.{options_str}""",
                     f"""Decide if there is hateful content in the given image.{options_str}""",
                     f"""Is there hateful intention in the given image?{options_str}""",
        ]
    elif task == 'medic_damage_severity':
        instructs = [
            f"""What is the damage level in the image?""",
            f"""Look at the image and decide the damage level. The damage level is given in options.{options_str}""",
            f"""Select the damage severity from options.{options_str}""",
            f"""In this task, you are required to decide how bad is the damage in the image. The levels of damage are severe, mild, and little or none.""",
            f"""Tell me how bad is the damage in the image.{options_str}"""
        ]
    elif task == 'medic_informative':
        instructs = [
            f"""Does this image provide any information about the disaster? Choose the correct answer from options.{options_str}""",
            f"""Is this picture informative about a disaster?{options_str}""",
            f"""Is this a informative picture of disaster?{options_str}""",
            f"""Can you gain any information about a disaster from the image? If yes, select informative, if not select not informative from the options.{options_str}""",
            f"""If this picture is about a disaster, select informative. Otherwise, select not informative from options.{options_str}""",
        ]
    elif task == 'medic_disaster_types':
        instructs = [
            f"""According to the image, what kind of disaster happened? Choose the correct answer from options.{options_str}""",
            f"""What kind of disaster happens in the image? If no disaster happens in the image, select not disaster.{options_str}""",
            f"""What disaster happens in the image?{options_str}""",
            f"""Based on the image, what is the disaster. Select your answer from options.{options_str}""",
            f"""Look at the image and tell me what kind of disater is in the image. If no disaster, select not disaster.{options_str}""",
        ]
